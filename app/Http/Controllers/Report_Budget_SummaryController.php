...
class Report_Budget_SummaryController extends Controller
{
    //

	function index(){

			  $status = DB::table('eprop_report_budget_summary_v')->get();
    ...
    // 	    ->get();
    	     ...
    	     $division = DB::table('eprop_report_budget_summary_v')
    	    ->select('division_name')
    	    ->groupby('division_name')
    	    ->get();

    	     $brand = DB::table('eprop_report_budget_summary_v')
    	    ->select('brand_name')
    	    ->groupby('brand_name')
    	    ->get();
			
			 $varian = DB::table('eprop_report_budget_summary_v')
    	    ->select('produk_name')
    	    ->groupby('produk_name')
    	    ->get();

    	     $activity_name = DB::table('eprop_report_budget_summary_v')
    	    ->select('activity_name')
    	    ->groupby('activity_name')
    	    ->get();

    // // 	      $type_market_code = DB::table('eprop_report_budget_summary_v')
    // // 	    ->select('type_market_code')
    // // 	    ->groupby('type_market_code')
    // // 	    ->get();

              $region = DB::table('eprop_report_budget_summary_v')
            ->select('region')
            ->groupby('region')
            ->get();

             $area = DB::table('eprop_report_budget_summary_v')
            ->select('area')
            ->groupby('area')
            ->get();


             $executor_name = DB::table('eprop_report_budget_summary_v')
            ->select('executor_name')
            ->groupby('executor_name')
            ->get();

              $company_name = DB::table('eprop_report_budget_summary_v')
            ->select('company_name')
            ->groupby('company_name')
            ->get();


              $tracker = DB::table('eprop_report_budget_summary_v')
            ->select('tracker')
            ->groupby('tracker')
            ->get();

             $status = DB::table('eprop_report_budget_summary_v')
            ->select('last_status')
            ->groupby('last_status')
            ->get();
       ...
       // echo "string";
          ...
          $tracker   = $request->tracker;
           ...
           $data = DB::table('eprop_report_budget_summary_v')
