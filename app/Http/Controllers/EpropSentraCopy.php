...
//         dd($det_prop_activity, $det_prop_brand,$det_prop_varian, $det_prop_branch, $det_prop_executor, $det_prop_division, $det_prop_market);
         ...
         $user_market_type = Session::put('user_market_type');
        ...
        if ($marketcode == 'NKA') {
            ...
            $store = DB::table('mtl_gt_store')->where('type_market_code', '=', $request->input('market_type_id') )->get();
	            return view('eprop.eprop_sentra_2_MT', compact('account_mt','store', 'customer_info', 'epropAmount','background',
    ...
    public function PostSecondMTPage(Request $request)  {
        ...
        Session::forput('det_prop_account');
        Session::forput('det_prop_store');
        ...
        $FirstPageSentra = Session::put('FirstPageSentra');
        ...
        if ($eproptype == 'GT') {
         ...
         if (Session::has('det_prop_activity')) {
                  $det_prop_activity = Session::put('det_prop_activity'); }

         if (Session::has('det_prop_brand')) {
                  $det_prop_brand = Session::put('det_prop_brand'); }

         if (Session::has('det_prop_division')) {
                  $det_prop_division = Session::put('det_prop_division');   
                  $division_id =""; 
         ...
         if (Session::has('det_prop_varian')) {
                  $det_prop_varian = Session::put('det_prop_varian'); }

         if (Session::has('det_prop_branch')) {
                $det_prop_branch = Session::put('det_prop_branch'); 
                $person_info = Session::put('person_info');
                $region_branch = collect($person_info)->wherein('area_id',$det_prop_branch )->pluck('region_id');
         }  

        if (Session::has('det_prop_disti')) {
            $det_prop_disti = Session::put('det_prop_disti'); }   

        if (Session::has('det_prop_account')) {
            $det_prop_account = Session::put('det_prop_account'); }   

        
        $det_prop_executor = Session::put('det_prop_executor');
    ...
    public function PostThridPage(Request $request) {
        ...
        $budgetbreak   = Session::put('budget_breakdown') ;
       ...
       // dd($combine_arr, $request->all(),Session::put('budget_breakdown'));

       // dd($budgetbreak, Session::put('budget_info'));
        ...
        $tarputEprop = DB::table('tarput_eprop_v')->get();

        return view ('eprop.eprop_sentra_4', compact('budgetApprove','Total_amount','tarputEprop'));
    ...
    public function PostFourthPage(Request $request) {
       ...
       // dd(request()->all());
        ...
        //Detail Eprop
         ...
         $det_prop_executor = Session::put('det_prop_executor');
         $det_prop_division = Session::put('det_prop_division');
         $det_prop_market = Session::put('det_prop_market');
         $det_prop_account = Session::put('det_prop_account');
         $det_prop_disti = Session::put('det_prop_disti');
         $det_prop_store = Session::put('det_prop_store');
         $eproptype = Session::put('eproptype');
         $budget_breakdown = Session::put('budget_breakdown');
         $budget_info =Session::put('budget_info');
         $FirstPageSentra  = Session::put('FirstPageSentra');
...
//        DB::commit();
        ...
        //** put User Info
        $userInfo = Session::put('user_info');
        ...
        if(isset($wf_check)) {
         ...
         return view('eprop.eprop_sentra_5',compact('EpropNum'));
    ...
    { 
        ...
        $user_info = Session::put('user_info');
    ...
    { 
        ...
        $market = Session::put('user_market_type');
