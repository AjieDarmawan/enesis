  	...
  	<div class="col-md-12">
  		...
  		<form class="form-horizontal form-group-md" method="POST" action="{{ route('budget_str.save_budget') }}">
  			...
  			<div class="form-group">
	            ...
	            <div class="col-xs-1">
	                ...
	                @foreach(Session::get('budget_master') as $value)
	                	<input type="text"  name="year" id="year" value = "{{ $value['budget_year'] }}" class="form-control" readonly="true" required/>
	                @endforeach
	                @endif
	            </div>
  			</div>
  			<div class="form-group">
	            ...
	            <div class="col-xs-2"> 
	                   <input type="text"  name="company_name" id="company_name" value = "{{ $company_name }}" class="form-control" readonly="true" required/> 
	            </div>   
	        </div>
	        <div class="form-group">
	            ...
	            <div class="col-xs-2">
	            	@if(Session::has('budget_master'))
	                @foreach(Session::get('budget_master') as $value)
	                	<input type="text" name="total_budget" id="total_budget"  value="{{ number_format($value['total_budget'], 2, ',', '.') }}" class="form-control" readonly="true" required/> 
	                @endforeach
	                @endif
	            </div>         
	        </div> 	   
